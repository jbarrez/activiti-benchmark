...
public class Benchmark {
  ...
  private static ProcessEngine getProcessEngine(String configuration, boolean historyEnabled) {
        if (configuration.equals("default")) {
            ...
            if (profiling) {
                ...
                ((ProcessEngineConfigurationImpl) processEngineConfiguration).setCustomPreCommandInterceptorsTxRequired(interceptors);
            }
    ...
    private static void executeBenchmarks(ProcessEngine processEngine, boolean historyEnabled, int nrOfProcessExecutions, int maxNrOfThreadsInThreadPool) {
  ...
  private static boolean readAndValidateParams(String[] args) {
      ...
      if (args.length != 2) {
          ...
          System.err.println();
          System.err.println("Usage: java -Xms512M -Xmx1024M -Dhistory={none|activity|audit|full} -Dconfig={default|spring} -Dprofiling -D{jdbcOption}={jdbcValue} -jar activiti-basic-benchmark.jar " + "<nr_of_executions> <max_nr_of_threads_in_threadpool> ");
          System.err.println();
      ...
      if (!System.getProperties().containsKey("history")) {
        System.out.println("No history config specified, using default value 'audit");
      ...
      if (configurationValue  != null && !configurationValue.equals("default") && !configurationValue.equals("spring")) {
          System.err.println("Invalid configuration option: only default|spring are currently supported");
          return false;
      }

      if (historyValue != null && !historyValue.equals("none") && !historyValue.equals("activity") && !historyValue.equals("audit") && !historyValue.equals("full")) {
